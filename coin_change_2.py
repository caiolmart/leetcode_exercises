...
class Solution:
    def change(self, amount: int, coins: List[int]) -> int:
        coins = [x for x in sorted(coins) if x <= amount]
        all_possible = [[0 for j in range(len(coins))] for i in range(amount + 1)]
        all_possible[0] = [1 for j in range(len(coins))]
        for a in range(1, amount + 1):
            for j, c in enumerate(coins):
                if c > a:
                    all_possible[a][j] = all_possible[a][j - 1]
                else:
                    all_possible[a][j] = all_possible[a - c][j]
                    if j > 0:
                        all_possible[a][j] += all_possible[a][j - 1]
        return all_possible[amount][len(coins) - 1]

if __name__ == "__main__":
    sol = Solution()
    print(sol.change(5, [1,2,5]))
    print(sol.change(500, [3,5,7,8,9,10,11]))
    amount = 4000
